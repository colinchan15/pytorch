...
from torch._C import DispatchKeySet, Stack
...
class GuardBuilder(GuardBuilderBase):
    ...
    def get_guard_manager_type(
        self, source: Source, example_value: Union[KeysView[Any], set[Any], frozenset[Any], dict[Any, Any]]
    ) -> GuardManagerType:
        guard_manager_enum = GuardManagerType.GUARD_MANAGER
            ...
            if self.check_fn_manager.output_graph.export:
                ...
                verbose_code_parts = get_verbose_code_parts(
                    get_tensor_guard_code_part(
                        value, tensor_name, size, stride, pytype, dispatch_keys  # type: ignore[arg-type]
                    ),
                    guard,
                )
...
class GuardsStatePickler(pickle.Pickler):
    ...
    @classmethod
    def _unpickle_mapping_proxy(cls, d: dict[Any, Any]) -> types.MappingProxyType[Any, Any]:
...
def get_guard_fail_reason_helper(
    ...
    """
    assert guard_manager.global_scope is not None and guard_manager.closure_vars is not None
